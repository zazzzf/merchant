...
_extend(KUploadButton, {
	...
	submit : function() {
		...
		formData.append("ssid", K.instances[0].ssid);
		formData.append("image", self.form[0][this.options.fieldName].files[0]);
		var resoult={
			error:1,
			message:"服务器通讯失败",
			url:""
		...
		$.ajax({
            ...
            .done((data)=>{
                ...
                if(!!data){
                    if(data.code==0){
                        resoult.error=0;
                        resoult.message=data.message;
                        resoult.url=data.data;
                    }else{
                    	resoult.message=data.message;
                    };
                    self.options.afterUpload.call(self, resoult);
                }else{
