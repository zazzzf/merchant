...
router.beforeEach((to, from, next) => {
    ...
    if (Cookies.get('locking') === '1' && to.name !== 'locking') {  // 判断当前是否是锁定状态
        ...
        if (!Cookies.get('ssid') && !Cookies.get('user') && !Cookies.get('merchant_id') && to.name !== 'login' && to.name !== 'register') {  // 判断是否未登录且前往的页面不是登录页
            next({
                name: 'login'
            });
        } else if (!!Cookies.get('ssid') && !!Cookies.get('user') && !!Cookies.get('merchant_id') && to.name === 'login') {  // 判断是否已经登录且前往的是登录页
            Util.title();
            next({
                name: 'home_index'
            });
...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        setPower (state, power) {
            let userRights = Cookies.get('merchant_type');
            console.log(userRights);
            if (!userRights) return;
            userRights = JSON.parse(userRights);

            state.Rights = userRights;
            Util.mixedRights(otherRouter.children, userRights.other, 'name', 'access', 'children');
            Util.mixedRights(appRouter, userRights.app, 'name', 'access', 'children');
        },
        updateMenulist (state) {
            let accessCode = parseInt(Cookies.get('access'));
            let merchant_type =parseInt(Cookies.get('merchant_type'));
            let menuList = []; //inline
            if(merchant_type===1){
                appRouter[1].access=1;
                appRouter[2].access=0;
            }else{
                appRouter[1].access=0;
                appRouter[2].access=1;
            }
...
Vue.filter('numberCarry', Util.numberCarry);
Vue.filter('fmoney', Util.fmoney);
